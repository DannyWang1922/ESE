...
parser.add_argument('--expert_dropout', type=float, default=0.1,
                    help='Expert dropout rate, default 0.1')
parser.add_argument('--router_temperature', type=float, default=0.1,
                    help='Router temperature for softmax, default 0.1')
parser.add_argument('--router_noise_epsilon', type=float, default=1e-2,
                    help='Noise epsilon for router during training, default 1e-2')
parser.add_argument('--router_training_noise', type=bool, default=True,
                    help='Whether to add noise during training, default True')
parser.add_argument('--use_load_balancing', type=bool, default=True,
                    help='Whether to use load balancing loss, default True')
parser.add_argument('--router_z_loss_coef', type=float, default=1e-3,
                    help='Coefficient for router z-loss, default 1e-3')
...
def load_and_process_train_data(args, tokenizer, max_length, prompt_template=None):
    ...
    for dataset_path in dataset_path_list:
        if os.path.exists(dataset_path):
            ds = load_dataset('json', data_files=[dataset_path], num_proc=args.workers, streaming=args.streaming)
        else:
            ds = load_dataset(dataset_path, args.train_subset_name, num_proc=args.workers, streaming=args.streaming)
        
        # Extract the split first
        ds_split = ds[args.train_split_name]
        ...
        print(ds_split)
        all_train_datasets.append(ds_split)
    ...
    if args.streaming:
        ...
        if args.max_train_samples is not None and len(combined_dataset) > args.max_train_samples:
            combined_dataset = combined_dataset.select(range(args.max_train_samples))
            logger.info(f'Limited combined dataset to {args.max_train_samples} samples')
    
    print("Combined_dataset: \n", combined_dataset)

    logger.info('Processing train...')
    train_ds = combined_dataset.shuffle(args.dataset_seed).map(
        AngleDataTokenizer(tokenizer, max_length, prompt_template=prompt_template),
...
def load_and_process_valid_data(args, tokenizer, max_length, prompt_template=None):
    ...
    if args.valid_name_or_path is None:
        return None
    ...
    # Extract the split first
    valid_ds_split = valid_ds[args.valid_split_name]
    
    # Apply max_valid_samples limit if specified (before map operations)
    if args.max_valid_samples is not None and not args.streaming:
        original_size = len(valid_ds_split)
        valid_ds_split = valid_ds_split.select(range(min(args.max_valid_samples, original_size)))
    ...
    print(valid_ds_split, "\n")   

    valid_ds_processed = valid_ds_split.map(
        AngleDataTokenizer(tokenizer, max_length, prompt_template=prompt_template),
        num_proc=args.workers)
